...
function changeColorStyleWithDefault(color) {
...
document.write('<iframe id=ColorFrame name=ColorFrame frameborder=0 width="209" width="177" src="includes/palette/showPalette.jsp" style=display:none;position:absolute;z-index:100></iframe>');


var color_old_onclick = document.onclick;
document.onclick = function() {
	if (color_old_onclick) color_old_onclick();
	hidePalette();
}

function showPalette(sTd,bOpenBound, sFld1, fld1Index, sFld2, sCallback, sForm,currentColor) {
	var fld1,fld2;
	var cf=document.getElementById("ColorFrame");
	var wcf=window.frames.ColorFrame;
	var oTd=document.getElementById(sTd);
	if(!sFld1){alert("Please specify the input control name, it's the third parameter of showPalette method.");return;}
	fld1=document.forms[sForm][sFld1];
	if(fld1.tagName!="INPUT") {
		fld1=fld1[fld1Index];
	}
	...
	//var eH=oTd.height,eW=oTd.width;
	var eH=25,eW=25;
	...
	wcf.fld1=fld1;
	wcf.fld2=fld2;
	wcf.DisColor.style.backgroundColor=changeColorStyleWithDefault(currentColor);
	wcf.HexColor.value =changeColorStyleWithDefault(currentColor);
