...
import net.sourceforge.web.struts.form.business.ta.AirTicketQueryForm;
...
public class AirTicketAction extends RechargeAction {
    ...
    private Map constructQueryMap(AirTicketQueryForm queryForm) {
        ...
        if (leave_datetime_from != null && leave_datetime_from.trim().length() > 0) {
            conditions.put(AirTicketQueryCondition.DEPART_TIME_LT, net.sourceforge.web.struts.action.ActionUtils.getQueryToDateFromDisplayDate(leave_datetime_from));
        }
        ...
        if (leave_datetime_to != null && leave_datetime_to.trim().length() > 0) {
            conditions.put(AirTicketQueryCondition.DEPART_TIME_GT, net.sourceforge.web.struts.action.ActionUtils.getQueryToDateFromDisplayDate(leave_datetime_to));
        }
        
        //set is on travel condition
        String is_on_travel = queryForm.getIs_on_travel();
        if (is_on_travel != null && is_on_travel.trim().length() > 0) {
            conditions.put(AirTicketQueryCondition.TA_IS_ON_TRAVEL_EQ, net.sourceforge.web.struts.action.ActionUtils.parseInt(is_on_travel));
        }                
