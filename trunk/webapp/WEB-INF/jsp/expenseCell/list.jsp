...
<!--
	function add() {
		v = window.showModalDialog(
			'showDialog.do?title=ExpenseCell.new.title&newExpenseCell.do' , 
	...
	function edit(id) {
		v = window.showModalDialog(
			'showDialog.do?title=ExpenseCell.edit.title&editExpenseCell.do?id=' + id , 
			null, 'dialogWidth:400px;dialogHeight:180px;status:no;help:no;scroll:no');
		if (v) {
...
<html:form action="listExpenseCell">
	...
	<!-- mtoIdList -->
			<td class="bluetext"><bean:message key="expenseCell.row_id" />:</td>
			<td>
				<html:select property="row_id">
				</html:select>
			</td>
			<td class="bluetext"><bean:message key="expenseCell.expenseSubCategory_id" />:</td>
			<td>
	...
	<!-- property -->
			...
			<td><html:text property="amount" size="8"/></td>

			<td align="left">
				<html:submit><bean:message key="all.query"/></html:submit>
				<input type="button" value="<bean:message key="all.new"/>"
					onClick="window.location.href='newExpenseCell.do" />
			</td>
		</tr>
	</table>
</html:form>
<hr />
<page:form action="listExpenseCell.do" method="post">
	<jsp:include page="../pageHead.jsp"/>
	<table class="data">
		<thead>
			<tr class="new_bg">
				<th width="30%"><page:order order="id" style="text-decoration:none">
					<bean:message key="expenseCell.id" />
					<page:desc>
				...
				<th width="70%"><page:order order="name"
					...
					<bean:message key="expenseCell.name" />
					<page:desc>
						<img src="images/down.gif" border="0" />
					</page:desc>
					<page:asc>
